...
export function generatePostContent(index: number, day: number): string {
    const safeIndex = isNaN(index) ? 0 : Math.max(0, index);
    const safeDay = isNaN(day) ? 1 : Math.max(1, day);
    
    let phase = CampaignPhases.RESEARCH;
    if (safeDay > 20) phase = CampaignPhases.MCP_ACTIVATION;
    else if (safeDay > 10) phase = CampaignPhases.FEEDBACK;

    const introArr = phase.intro || CampaignPhases.RESEARCH.intro;
    const hooksArr = phase.hooks || CampaignPhases.RESEARCH.hooks;
    const ctaArr = phase.cta || CampaignPhases.RESEARCH.cta;

    const i = safeIndex % introArr.length;
    const h = Math.floor(safeIndex / introArr.length) % hooksArr.length;
    const c = Math.floor(safeIndex / (introArr.length * hooksArr.length)) % ctaArr.length;

    const intro = introArr[i] || "Protocol update is live.";
    const hook = hooksArr[h] || "Developing agentic finance on Starknet.";
    const cta = ctaArr[c] || "Scouting for peers on Sepolia. 🦞";

    return `${intro} ${hook} ${cta}`;
}
